...
import type { Asset, AssetMovement, AssetTargetMode, AssetType, AssetUpdateSource, MovementType } from '../types';
...
export default function AssetManager() {
  ...
  const selectSuggestion = (s: DbSuggestion) => {
    setForm(prev => ({
      ...
      tickerCodigo: s.tickerCodigo ?? prev.tickerCodigo,
      cnpj: s.cnpj ? formatCnpj(s.cnpj) : prev.cnpj,
      precoUnitario: s.precoUnitario !== undefined ? String(s.precoUnitario) : prev.precoUnitario,
  ...
  const handleImportFile = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      for (const row of rows) {
        ...
        const tipo = (normalizeKey(String(normalized.tipo ?? 'outro')) || 'outro') as AssetType;
        ...
        <Modal title={editId ? 'Editar Ativo' : 'Novo Ativo'} onClose={() => setShowModal(false)} size="lg">
          <div className="grid gap-3 md:grid-cols-2">
            ...
            <div>
              ...
              <select value={form.tipo} onChange={e => setForm(prev => ({ ...prev, tipo: e.target.value as AssetType }))} className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm">
                {['acao','fii','etf','bdr','fundo','cdb','cri','cra','debenture','coe','cripto','conta_corrente','valores_em_transito','outro'].map(type => <option key={type} value={type}>{type}</option>)}
              </select>
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Ticker/Codigo</label>
              <input value={form.tickerCodigo} onChange={e => setForm(prev => ({ ...prev, tickerCodigo: e.target.value }))} className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm" />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Codigo (Renda Fixa)</label>
              <input value={form.identificadorExterno} onChange={e => setForm(prev => ({ ...prev, identificadorExterno: e.target.value }))} placeholder="Ex: CRA02300CYT" className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm" />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">CNPJ</label>
              <input value={form.cnpj} onChange={e => setForm(prev => ({ ...prev, cnpj: e.target.value }))} className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm" />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">ISIN</label>
              <input value={form.isin} onChange={e => setForm(prev => ({ ...prev, isin: e.target.value }))} className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm" />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Identificador Externo</label>
              <input value={form.identificadorExterno} onChange={e => setForm(prev => ({ ...prev, identificadorExterno: e.target.value }))} className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm" />
            </div>
            <div>
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Origem atualizacao</label>
              <select value={form.origemAtualizacao} onChange={e => setForm(prev => ({ ...prev, origemAtualizacao: e.target.value as AssetUpdateSource }))} className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm">
                {['manual','importacao_excel','extrato_btg','api','outro'].map(origin => <option key={origin} value={origin}>{origin}</option>)}
              </select>
            </div>
